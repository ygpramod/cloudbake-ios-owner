    ...
    func save(_ item: InventoryItem, in db: Database) throws {
        try db.execute(
            sql: """
                ...
                (id, name, aliases_json, inventory_type, default_expiry_days, unit, current_quantity, minimum_quantity, created_at_unix_time, updated_at_unix_time, archived_at_unix_time)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    func save(_ batch: InventoryStockBatch, in db: Database) throws {
        try db.execute(
            sql: """
                ...
                (id, inventory_item_id, remaining_quantity, expires_at_unix_time, amount_decimal, unit_cost_decimal, created_at_unix_time, updated_at_unix_time)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
